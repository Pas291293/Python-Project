...
class history():      
    ...
    def set_history(self,history_dictionary):
        new_key=list(history_dictionary.keys()) ## returns the key from dictionary
        new_key=str(new_key)[2:-2]  #string slicing
        value=history_dictionary.get(new_key)  #getting only the values for the key
        return value
...
def get_accuracy_per_epoch(history=history_obj):
    ...
    res=dict(zip(d,d2))  ##Zip() returns zip object which is an parallel iterator of tuples
    return res